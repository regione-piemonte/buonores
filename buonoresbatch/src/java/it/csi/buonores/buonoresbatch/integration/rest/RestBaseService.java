...
******************************************************************************/
package it.csi.buonores.buonoresbatch.integration.rest;
...
public class RestBaseService extends BatchLoggerFactory {
	...
	public static RestBaseService getInstance(BuonoresBatchDAO dao) {
		if (instance == null) {
	...
	private OkHttpClient getHttpClientBandi() {
		if(okHttpClient == null) {
			okHttpClient = new OkHttpClient.Builder()
			        ...
			        .writeTimeout(45, TimeUnit.SECONDS)
			        .readTimeout(45, TimeUnit.SECONDS)
			        .addInterceptor(new BasicAuthInterceptor(this.buonoresbandisrvusername, this.buonoresvbandisrvpassword))
			        .build();
		}
	...
	public ResponseRest eseguiGet(String cf,String url, boolean shibboleth, List<String> headerName, boolean iride) throws Exception {
	...
	public ResponseRest eseguiPost(String cf, String url, String jsonBody, boolean shibboleth,boolean iride) throws Exception{
		
		Request request = createPostRequest(cf, url, jsonBody, shibboleth,iride);
	...
	private Request createGetRequest(String cf,String url, boolean shibboleth, boolean iride){
		Request request = addCustomHeaders(cf,Constants.XREQUESTID,Constants.XFORWARDEDFOR,Constants.XCODICESERVIZIO, new Request.Builder(), shibboleth, iride)
				.url(url)
				.build();
	...
	private Request createPostRequest(String cf, String url, String json, boolean shibboleth, boolean iride) {
		RequestBody body = RequestBody.create(json, MediaType.parse("application/json; charset=utf-8"));
		Request request = addCustomHeaders(cf, Constants.XREQUESTID, Constants.XFORWARDEDFOR,
				Constants.XCODICESERVIZIO, new Request.Builder(), shibboleth, iride).url(url).post(body).build();

		return request;
	}
	
	private Request.Builder addCustomHeaders(String cf,String xRequestId, String xForwardedFor, String xCodiceServizio, Request.Builder reqBuilder, boolean shibboleth,boolean iride) {

		reqBuilder.addHeader(Constants.X_CODICE_SERVIZIO,xCodiceServizio)
		.addHeader(Constants.X_REQUEST_ID, xRequestId);
		if(iride) {

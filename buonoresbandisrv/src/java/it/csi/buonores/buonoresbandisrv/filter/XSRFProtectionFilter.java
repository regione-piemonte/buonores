...
******************************************************************************/
package it.csi.buonores.buonoresbandisrv.filter;
 ...
 * di tipo
 * XSRF.
 * L'idea e' che il primo accesso avvenga sulla risorsa di accesso della
 * componente SPA
 * (es. index.html). In quell'occasione il filtro non fa verifiche ma si
 * preoccupa di
 * creare una nuova sessione servlet se non presente, creare un XSRF cookie,
 * metterlo in
 * sessione e passarlo al client.
 * Nelle successive richieste, se la rihciesta e' destinata allo strato di
 * servizi rest,
 * viene effettuata la verifica di presenza/corrispondenza di cookie+header
 * XSRF.
 * Questo comportamento, unito alla corretta gestione client side del
 * protocollo,
 * permette di evitare il richiamo dello strato rest al di fuori del contesto
 * corretto.
 */
public class XSRFProtectionFilter implements Filter {
	 ...
	 * nella
	 * cache del browser per evitare che, ad un accesso in una sessione successiva,
	 * la fase
	 * di ingresso (e quindi di ricreazione del token) venga bypassata. Se cio'
	 * accadesse
	 * le richieste REST sarebbero le prime ad arrivare al server e si ricadrebbe
	 * nella
